// CS 131: nth-prime using Grand Central Dispatch
//
...
int main(int argc, char **argv)
{
    ...
    dispatch_queue_t serialQ =
                        dispatch_queue_create("serialQ", DISPATCH_QUEUE_SERIAL);

    dispatch_queue_t parallelQ =
                  dispatch_queue_create("parallelQ", DISPATCH_QUEUE_CONCURRENT);

    dispatch_group_t forG = dispatch_group_create();

    for (uint64_t r = rangeStart; r < rangeEnd; r += step) {
      uint64_t rnext = r+step;
      if (rnext > rangeEnd)
          rnext = rangeEnd;
      dispatch_group_async(forG, parallelQ, ^{
              __block uint64_t cp = countPrimes(r, rnext, 0, NULL);
          dispatch_group_async(forG, serialQ, ^{
            count += cp;
          });
        });
    }
    dispatch_group_wait(forG, DISPATCH_TIME_FOREVER);
